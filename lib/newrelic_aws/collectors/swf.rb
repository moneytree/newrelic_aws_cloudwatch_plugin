      ...
      def domains
        ...
        swf.domains.map { |domain|
          ...
          if domain.name == "Scraper"
            ...
            completedExecutionsLong = domain.workflow_executions.closed_after(workflow_start_time).with_status("COMPLETED")
            completedExecutionsLong.each do |execution|
              completedCount += 1
